"""Handler for /playmatch — full match with live scorecard, @mentions, new batsman after wicket."""
...
def _pd(e, p):
    return {
        "roster_id": e.id, "player_id": p.id, "name": p.name, "rating": p.rating,
...
def _bowl_label(p, s):
    """Bowler button label with hand, style and match stats."""
    bws = s["bowl_stats"].get(p["roster_id"], {})
    od = bws.get("overs_done", 0)
    tb = bws.get("this_over_balls", 0)
    ov_str = f"{od}.{tb}" if tb else str(od)
    runs = bws.get("runs", 0)
    wkts = bws.get("wickets", 0)
    hand = p.get("bowl_hand", "R")[:1]
    return f"{p['name']} | {hand}-{p['bowl_style']} | {ov_str}•{runs}•{wkts}"


def _bat_label(p, s):
    """Batsman button label with runs(balls)."""
    bs = s["bat_stats"].get(p["roster_id"], {})
    return f"{p['name']} — {bs.get('runs', 0)}({bs.get('balls', 0)})"
...
async def playmatch_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Usage: /playmatch @username"); return
    t = context.args[0].lstrip("@").strip()
    session = get_session()
    try:
        u1 = session.query(User).filter(User.telegram_id == tg.id).first()
        if not u1: await update.message.reply_text("❌ Do /debut first!"); return
        ...
        if r1 < 11: await update.message.reply_text(f"❌ You need 11+ players ({r1})."); return
        ...
        m = Match(user1_id=u1.id, user2_id=u2.id, status="pending",
                  ...
                  chat_id=cid, created_at=now, expires_at=now + timedelta(seconds=MATCH_EXPIRE))
        session.add(m); session.commit()
        t1 = u1.team_name or f"@{u1.username}'s XI"; t2 = u2.team_name or f"@{u2.username}'s XI"
        txt = (f"🔔 <b>NEW MATCH INVITATION!</b>\n\nFrom: @{u1.username} to @{u2.username}\n\n"
               f"🏏 <b>CRICKET GURU MATCH</b>\n\n{t1} vs {t2}\n"
               f"📍 Pitch: {st['pitch_type']}\n🌤️ {st['weather']} | 🌡️ {st['temperature']}°C\n"
        ...
        try:
            if context.job_queue:
                context.job_queue.run_once(_auto_expire, MATCH_EXPIRE, name=f"match_{m.id}",
                                           data={"match_id": m.id, "chat_id": cid})
        except Exception: pass
    except Exception:
        session.rollback(); logger.exception("Playmatch err"); await update.message.reply_text("⚠️ Error.")
...
async def match_accept_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not u or u.id != auid: await q.answer("Only invited player!"); return
        await q.answer()
    ...
    except Exception: session.rollback(); logger.exception("Accept err")
    finally: session.close()
...
async def overs_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(cid, f"🪙 <b>TOSS!</b>\n\n🏆 @{w.username} wins!\n\n@{w.username}, choose:", parse_mode="HTML", reply_markup=kb)
...
async def toss_decision_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        m.toss_decision = dec
        if dec == "bat":
            m.batting_first_id = wid; m.bowling_first_id = m.user2_id if wid == m.user1_id else m.user1_id
        else:
            m.bowling_first_id = wid; m.batting_first_id = m.user2_id if wid == m.user1_id else m.user1_id
        m.status = "selecting"; session.commit()
        ch = "BAT FIRST" if dec == "bat" else "BOWL FIRST"
        await q.edit_message_text(f"✅ @{u.username} elected to {ch}")
        ...
        # Show Playing XIs using the 5-section format
        bat_roster = (session.query(UserRoster, Player).join(Player, UserRoster.player_id == Player.id)
                      .filter(UserRoster.user_id == bu.id).order_by(UserRoster.order_position).limit(11).all())
        bowl_roster = (session.query(UserRoster, Player).join(Player, UserRoster.player_id == Player.id)
                       .filter(UserRoster.user_id == bwu.id).order_by(UserRoster.order_position).limit(11).all())

        bat_text = format_xi_text(bat_roster, f"🏏 {bt} (Batting)", bu.captain_roster_id)
        bowl_text = format_xi_text(bowl_roster, f"🎳 {bwt} (Bowling)", bwu.captain_roster_id)

        await context.bot.send_message(cid, bat_text, parse_mode="HTML")
        await context.bot.send_message(cid, bowl_text, parse_mode="HTML")
...
async def opener2_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Build bowler buttons with hand & style
        ...
        btns = [[InlineKeyboardButton(
            f"{p['name']} | {p.get('bowl_hand','R')[:1]}-{p['bowl_style']}",
...
async def _show_delivery(ctx, cid, mid):
    ...
    hdr = (f"🎳 <b>OVER {ov} • BALL {bl}</b>\n\n"
           ...
           f"📍 {ph}\n\n━━━━━━━━━━━━━━━━━━━\n\n"
           f"@{s['bowl_username']}, choose your delivery:\n\n")
    if opts["is_spinner"]:
...
async def spinner_delivery_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "Surprise":
        ns = [x for x in opts["deliveries"] if x != "Surprise"]; d = random.choice(ns) + " (Surprise)"
...
async def _show_shot(ctx, cid, mid):
    ...
    txt = (f"🏏 <b>OVER {s['current_over']} • BALL {s['current_ball'] + 1}</b>\n\n"
           ...
           f"━━━━━━━━━━━━━━━━━━━\n\n"
           f"@{s['bat_username']}, choose your shot:")
    btns = []; row = []
...
async def shot_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    legal = True; need_new_batsman = False
    ...
    if oc["type"] == "wide":
        ...
        sym = SYM["NB"] + (SYM.get(runs, str(runs)) if runs > 0 else "")
        add_to_timeline(s, sym); legal = False
        ...
        sym = str(runs) + " 𓂾" if runs > 1 else "𓂾"
        add_to_timeline(s, sym)
        ...
        if runs == 0: rtxt = "0️⃣ <b>DOT BALL!</b>"
    ...
    if s["current_ball"] >= 6:
        ...
        s["prev_bowler_rid"] = bowler["roster_id"]; end_of_over = True

    _ss(context, mid, s)

    sc = build_live_scorecard(s)
    msg = f"🎳 {bowler['name']} → {dl}\n🏏 {striker['name']} played {shot}\n\n{rtxt}\n\n{sc}"
    await q.edit_message_text(msg, parse_mode="HTML")

    if is_innings_over(s):
        await _end_innings(context, mid); return

    if need_new_batsman and s["total_wickets"] < 10:
...
def _calc(s, striker, bowler, shot, delivery):
    bat_r = striker["bat_rating"]; bowl_r = bowler["bowl_rating"]
    diff = bat_r - bowl_r; r = random.random() * 100
    ...
    if r < 8 + wck:
        return {"type": "wicket", "runs": 0, "how": random.choice(["Bowled", "Caught", "LBW", "Caught Behind", "Caught & Bowled", "Stumped"])}
...
async def _show_new_batsman(ctx, mid):
    s = _gs(ctx, mid)
    if not s: return
    # Find available batsmen (not yet out, not currently at crease)
    crease_rids = {s["batting_order"][s["striker_idx"]]["roster_id"],
                   s["batting_order"][s["non_striker_idx"]]["roster_id"]} if s["total_wickets"] < 10 else set()

    available = []
    for i, p in enumerate(s["batting_order"]):
        if i == s["striker_idx"] or i == s["non_striker_idx"]:
            continue
        bs = s["bat_stats"].get(p["roster_id"], {})
...
async def new_batsman_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; tg = q.from_user
    parts = q.data.split("_"); mid, bat_idx = int(parts[1]), int(parts[2])
    s = _gs(context, mid)
    if not s or tg.id != s["bat_user_tg"]: await q.answer("Not yours!"); return
    await q.answer()

    new_bat = s["batting_order"][bat_idx]
    s["striker_idx"] = bat_idx
    _ss(context, mid, s)

    await q.edit_message_text(
        f"🏏 New batsman: {new_bat['name']} ({new_bat['bat_rating']} BAT)", parse_mode="HTML")

    if is_innings_over(s):
        await _end_innings(context, mid); return

    # Check if we also need a new bowler (end of over coincided with wicket)
...
async def _show_new_over_bowler(ctx, mid):
    ...
    btns = [[InlineKeyboardButton(
        _bowl_label(p, s),
        callback_data=f"nbowl_{mid}_{p['roster_id']}"
    )] for p in avail[:8]]

    await ctx.bot.send_message(s["chat_id"],
        f"🎳 <b>OVER {s['current_over']}</b> — Select bowler:\n"
        f"📊 {format_score(s)} | {format_overs(s)} ov\n\n@{s['bowl_username']}, choose:",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(btns))
...
async def _end_innings(ctx, mid):
    ...
    if s["innings"] == 1:
        ...
        target = s["target"]; chasing = s["total_runs"]
        if chasing >= target:
            winner = s["bat_team_name"]; margin = f"by {10 - s['total_wickets']} wickets"
        else:
            winner = s["bowl_team_name"]; margin = f"by {target - 1 - chasing} runs"
        await ctx.bot.send_message(cid,
            f"━━━━━━━━━━━━━━━━━━━\n🏆 <b>MATCH RESULT</b>\n\n"
            f"🔴 {s['inn1_team']}: {s['inn1_runs']}/{s['inn1_wickets']} ({s['inn1_overs']})\n"
            f"🟢 {s['bat_team_name']}: {format_score(s)} ({format_overs(s)})\n\n"
            f"🏆 <b>{winner} wins {margin}!</b>\n━━━━━━━━━━━━━━━━━━━", parse_mode="HTML")
