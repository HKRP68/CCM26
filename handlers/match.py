    ...
    try:
        ...
        try:
            ...
            if st.get("stats_disabled"):
                ...
                await context.bot.send_message(
                    cid,
                    "⚠️ <b>Player stats WON'T count this match.</b>\n"
                    f"Team Overall gap is too wide — {bat_team} <b>{b_ovr}</b> "
                    f"vs {bowl_team} <b>{bw_ovr}</b> "
                    f"(<b>{abs(b_ovr - bw_ovr)}</b> apart, limit "
                    f"{STATS_FAIRNESS_OVR_GAP}). Career batting &amp; bowling "
                    "stats are skipped to keep the game fair and stop "
                    "stat-farming. The match still plays out as normal.",
                    parse_mode="HTML")
        except Exception:
