...
def _migrate_add_columns():
    ...
    def _try_add(table, col, coltype):
        ...
        for sql in (
            ...
            try:
                ...
                last_err = e
                err = str(e).lower()
                # "already exists" / "duplicate column" → column is there, success
                if ("already exists" in err or "duplicate column" in err
                        or "duplicate_column" in err):
                    return
                continue
        # All attempts failed AND it wasn't an "already exists" — log so the
        # admin sees a real problem instead of silent corruption.
        if last_err:
            import logging
            logging.getLogger(__name__).warning(
                f"migration: ALTER TABLE {table} ADD COLUMN {col} {coltype} "
                f"failed on all forms: {last_err}"
            )
    ...
    # Backfill existing rows (DEFAULT only applies to new inserts in some
    # DBs). Treat existing progress as "assigned" so users keep their
    # old quests visible until their period rolls.
    try:
        with engine.begin() as conn:
            conn.execute(text(
        ...
        import logging
        logging.getLogger(__name__).warning(
            f"backfill assigned column skipped: {e}"
        )
