...
def webapp():
    ...
    ``ad_config`` carries the block id so the controller can be built without
    waiting for /api/webapp/init. The SDK script tag itself is unconditional in
    the template — see ``ad_service.client_config``.
    """
...
def webapp_init():
    ...
    try:
        ...
        return {
            ...
            # `ads` is the current shape; `adsgram` is the legacy one, kept so
            # a Mini App page cached in a Telegram WebView keeps working.
...
# Adsgram sends the Telegram id as `userid`. The aliases cost nothing and make
# a mistyped reward URL work rather than silently dropping every postback.
_POSTBACK_ID_PARAMS = ("userid", "telegram_id", "tgid", "user_id")


@app.route("/api/ads/config", methods=["GET"])
@csrf_exempt
def ads_config():
    """Public: the Adsgram block id for the client SDK.
    ...
    Nothing secret is in it — the block id is public by definition; it ships in
    every page that shows an ad.
    """
    from services import ad_service
...
def ads_postback():
    ...
    Adsgram fires a GET here after a user finishes watching a rewarded ad.
    ...
    Configure the reward URL in your network's dashboard as:
      ...
      https://your-app.example.com/api/ads/reward?userid=[userId]

    Adsgram substitutes ``[userId]`` with the Telegram id before making the
    request. ``/api/adsgram/reward`` points here too, so a block configured
    with the older URL keeps working untouched.

    No auth — Adsgram doesn't sign these requests; the security model is
...
EVENT_KEYS = [
    ...
    ("career_chase_runs", "🎖🏏 Runs scored batting second (cumulative)"),
    ("career_boundary_runs", "🎖🏏 Runs taken in fours and sixes (cumulative)"),
    ("career_quickfire_runs", "🎖🏏 Runs from innings struck at 150+ over 10+ balls"),
    ("career_fours_hit", "🎖🏏 Fours hit by your Career Player (cumulative)"),
    ("career_sixes_hit", "🎖🏏 Sixes hit by your Career Player (cumulative)"),
    ("career_balls_faced", "🎖🏏 Balls faced by your Career Player (cumulative)"),
    ("career_fifty", "🎖🏏 Career Player scored 50+ in a match"),
    ("career_hundred", "🎖🏏 Career Player scored 100+ in a match"),
    ("career_score_25_plus", "🎖🏏 Career Player scored 25+ in an innings"),
    ("career_score_75_plus", "🎖🏏 Career Player scored 75+ in an innings"),
    ("career_not_out", "🎖🏏 Career Player finished an innings not out"),
    ("career_wickets_taken", "🎖🎳 Wickets taken by your Career Player (cumulative)"),
    ("career_dot_balls", "🎖🎳 Dot balls bowled by your Career Player (cumulative)"),
    ("career_maiden_overs", "🎖🎳 Maiden overs bowled by your Career Player (cumulative)"),
    ("career_balls_bowled", "🎖🎳 Balls bowled by your Career Player (cumulative)"),
    ("career_three_fer", "🎖🎳 Career Player took 3+ wickets in a match"),
    ("career_five_fer", "🎖🎳 Career Player took 5+ wickets in a match"),
    ("career_hattrick", "🎖🎳 Career Player took a hat-trick"),
    ("career_economy_spell", "🎖🎳 Career Player bowled 4+ overs under 6 an over"),
...
_ALLOWED_CHALLENGE_IMAGE_EXTS = {"png", "jpg", "jpeg", "webp", "gif"}


def _normalize_admin_command(value):
...
def _overseas_form(name):
    """Read an optional 0..11 overseas limit from the form.

    An empty box means "inherit" and comes back as ``None`` — distinct from a
    typed 0, which is the real rule "no overseas players at all".
    """
    raw = (request.form.get(name) or "").strip()
    if raw == "":
        return None
    try:
        return max(0, min(11, int(raw)))
    except (TypeError, ValueError):
        return None


def _tg_id_form(name):
    """Read an optional Telegram id from the form; blank/invalid comes back None."""
    raw = (request.form.get(name) or "").strip()
    if not raw:
        return None
    try:
...
def _ensure_default_challenge_mode(db):
    mode = (db.query(ChallengeMode)
              .order_by(ChallengeMode.sort_order, ChallengeMode.id)
              .first())
    if mode:
        return mode
                ...
                if action == "add_league":
                    ...
                    if not name:
                        ...
                        league = ChallengeLeague(
                            ...
                            fixtures_command=_normalize_admin_command(request.form.get("fixtures_command")) or None,
        ...
        if request.method == "POST":
            ...
            try:
                if action == "save_settings":
                    ...
                    # Overseas-in-XI rule. Blank means "inherit the league's own
                    # limit", which is why these are not run through _int_form
                    # (it would turn an empty box into 0 — a real, much stricter
                    # rule).
                    t.min_overseas = _overseas_form("min_overseas")
                    t.max_overseas = _overseas_form("max_overseas")
                    if (t.min_overseas is not None and t.max_overseas is not None
                    ...
                    if not tt or tt.tournament_id != t.id:
                        ...
                        tournament_service.set_team_owner(
                            db, tt.id, _tg_id_form("owner_tg_id"),
                            request.form.get("owner_name"))
                        log_admin(db, "tournament_team_owner", "tournament", t.id,
                                  f"{tt.name} → {tt.owner_tg_id or 'unowned'}")
                        flash(f"✅ {tt.name}: "
                    ...
                    if not tt or tt.tournament_id != t.id:
                        ...
                        pitch = (request.form.get("home_pitch") or "").strip()
                    ...
                    flash((f"✅ Inherited {n} team owner(s) from the draft.") if n
                          else "No new owners to inherit — every team already has "
                               "one, or this league did not come from a draft.",
                          "info" if not n else "success")
                elif action == "assign_venues":
                    from services import league_schedule_service
                    n = league_schedule_service.assign_fixture_venues(
                        db, t.id, overwrite=_checked("overwrite_venues"))
                    log_admin(db, "tournament_assign_venues", "tournament", t.id, t.name)
                    flash(f"✅ Updated {n} fixture venue field(s)." if n
                          else "Every unplayed fixture already has its venue set.",
                          "success" if n else "info")
                    ...
                    try:
                        ...
                        league_schedule_service.set_fixture_pitch(
                            db, fx.id, request.form.get("pitch_type"))
                        flash("✅ Fixture pitch updated.", "success")
                    except ValueError as ve:
                        db.rollback()
                        flash(f"⚠️ {ve}", "error")
                    ...
                    try:
                        ...
                        league_schedule_service.set_fixture_home(
                            db, fx.id, _int_form("home_team_id") or None)
                        flash("✅ Home team updated.", "success")
                    except ValueError as ve:
                        db.rollback()
                        flash(f"⚠️ {ve}", "error")
